...
public final class CodeReviewPrompts {
    ...
    public static String projectOverviewFinalSystem() {
        return """
                ...
                ⚠️ 关键输出规则（违反任何一条你的结果会被丢弃重试）：

                1. **禁止重复输出同一段内容**。summaryMd 和 mermaidCode 只能出现一次完整版本，
                   绝对不要写完一遍后再重新写一遍（包括在同一字段的字符串值里），
                ...
                3. **对项目中的深度子系统必须展开细讲**。如果项目里有以下任何一项，请在
                   ...
                   - 代码分析 / 规约库管理 / Finding 反馈闭环 / 误判防护机制
                   - 多 Agent 协作 / RAG 管道 / 向量检索
                   - 发布管道 / 定时任务 / 分布式锁
                   - 评估中心 / A/B 对比
                   - Guardrails / 安全护栏
                   - 大的基础设施（比如自研的异步执行器、LLM 路由）

                4. **严格输出 JSON 对象**，不要 markdown 围栏（不要 ```json 开头也不要 ``` 结尾），
                   不要任何前后缀文字（包括"以下是..."这类话）。
                ...
                JSON 结构：
                {
                  ...
                  "summaryMd": "Markdown 长文，必须含这些章节且每章只写一次：\\n## 项目定位\\n## 核心功能（列出 5-10 个能看到的能力，深度子系统单独展开）\\n## 技术选型（每一项说明"用什么 + 为什么"）\\n## 模块分层（基于摘要逐个介绍每个模块，每个模块 2-3 段）\\n## 典型数据流（至少画出 1-2 条端到端流程）\\n## 关键文件导读（列 10-20 个新人必读文件及路径 + 原因）\\n## 部署与运行\\n## 潜在改进 / 坑",
                  "mermaidCode": "graph TD 或 sequenceDiagram 语法，30-80 行，画出模块依赖或核心数据流。只能有一个 graph/sequenceDiagram 声明"
                }
